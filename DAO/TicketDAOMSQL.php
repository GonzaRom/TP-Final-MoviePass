...
class TicketDAOMSQL
{
    ...
    public function getByCinema($purchase, $cinema)
    {
        $tickets = array();
        try {
            $sql = 'SELECT * FROM ' . $this->tablename  . ' as p INNER JOIN movieshows as m ON p.idmovieshow = m.idmovieshow INNER JOIN typemovieshows as tm 
                ...
                ON c.idcinema = m.idcinema
                INNER JOIN typerooms as t 
                ON r.idtyperoom = t.idtyperoom INNER JOIN seats as s ON p.idseat = s.idseat  WHERE p.idpurchase = :id AND m.idcinema = :cinema';
            $parameters['id'] = $purchase;
